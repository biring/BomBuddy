...
def scrub_cbom_for_cost_walk() -> None:
    ...
    raw_df = frames.remove_unwanted_build_cost_data(raw_df)

    # *** Extract cbom table ***
    # drop rows above cBOM header
    cbom_df = frames.drop_rows_above_bom_header(raw_df)
...
def scrub_cbom_for_stella_db_upload() -> None:
    ...
    bom_df = frames.drop_rows_above_bom_header(raw_df)
    # set the top row as header
    ...
    data_frame = frames.drop_unwanted_stella_cbom_description_items(data_frame)
    ...
    data_frame = frames.drop_unwanted_cbom_component_type_items(data_frame)
...
def scrub_ebom_for_stella_db_upload():
    ...
    data_frame = frames.cleanup_description(data_frame)
    # remove rows that have unwanted description items
    data_frame = frames.drop_unwanted_stella_ebom_description_items(data_frame)
